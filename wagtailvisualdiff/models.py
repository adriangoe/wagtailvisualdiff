...
class PageRevisionScreenshot(models.Model):
    ...
    screenshot = models.ImageField(
        null=True, blank=True, max_length=254, storage=ScreenshotStorage()
    )
    mobile_screenshot = models.ImageField(
        null=True, blank=True, max_length=254, storage=ScreenshotStorage()
    )
    diff = models.ImageField(
        null=True, blank=True, max_length=254, storage=ScreenshotStorage()
    )

    def get_current(self):
        current = PageRevision.objects.get(
            pk=self.page_revision.id
        ).page.get_latest_revision()
        try:
            current_screenshot = PageRevisionScreenshot.objects.get(
                page_revision=current.pk
            )
