...
def process_page_published(instance_id, revision_id):
    ...
    params = {"fullpage": "1", "width": "700", "delay": "5", "force": "1"}
    ...
    while True:
        ...
        if fp2.getcode() == 200:
            try:
                prs.mobile_screenshot.save(
                    str(revision.id) + "_mobile.png", ContentFile(fp2.read())
                )
                break
            except SocketError as e:
                if e.errno not in [errno.ECONNRESET, errno.ETIMEDOUT, errno.EHOSTDOWN]:
...
def get_changes(previous_id, revision_id):
    ...
    for element in raw_diff:
        if str(element) == "$insert":
            ...
            changes.append({"title": "Added Blocks", "fields": fields, "color": "good"})
            ...
            changes.append(
                {"title": "Removed Blocks", "fields": fields, "color": "danger"}
            )
        else:
            ...
            try:
                for value in block_diff:
                    fields.append({"value": str(value), "short": True})
                block_type = str(b_json[element]["type"])
            except:
                block_type = (
                    str(raw_diff[element]["type"][0])
                    + " to: "
                    + str(raw_diff[element]["type"][1])
                )
                fields.append({"value": "Complicated Changes: Details in Link"})
            changes.append(
                {
    ...
    if not changes:
        changes.append({"title": "No fields were changed", "color": "good"})
    return changes
...
def screenshotlayer(url, args):
    ...
    return settings.SCREENSHOTLAYER_URL + "?access_key=%s&secret_key=%s&%s" % (
        access_key,
        secret_key,
        query,
    )
...
def get_diff(prs):
    ...
    diff_url = settings.HOSTNAME + "/wagtailvisualdiff/%s/for_sc" % str(
        prs.page_revision.id
    )
    params = {
        "fullpage": "1",
        "width": "900",
    ...
    return InMemoryUploadedFile(
        thumb_io, None, "diff.png", "image/jpeg", thumb_io.len, None
    )
...
def send_slack_notification(name, user, prs):
    ...
    if index < len(aslist) - 1:
        ...
        try:
                ...
                "attachments": [
                    {
                        ...
                        "title": "Diff between Revision "
                        + str(previous.page_revision.id)
                        + " and "
                        + str(prs.page_revision.id),
                        "title_link": "%s/wagtailvisualdiff/%s/to_previous"
                        % (settings.HOSTNAME, str(prs.page_revision.id)),
            ...
            values = {
                ...
                + " was published by "
                + user
                + ". Unfortunately we are missing a screenshot, but here is the detailed Diff: %s/wagtailvisualdiff/%s/to_previous"
                % (settings.HOSTNAME, str(prs.page_revision.id)),
    ...
    requests.post(
        settings.SCREENSHOT_SLACK_WEBHOOK, data=json.dumps(values), headers=headers
    )
...
def process_page_unpublished(instance_id):
    ...
    values = {
        ...
        "icon_url": settings.SCREENSHOT_BOT_SLACK_ICON,
        "text": instance.title
        + " was unpublished! %s/cms/admin/pages/%s/edit/"
        % (settings.HOSTNAME, instance_id),
    }
    headers = {"Content-type": "application/json"}
    requests.post(
        settings.SCREENSHOT_SLACK_WEBHOOK, data=json.dumps(values), headers=headers
